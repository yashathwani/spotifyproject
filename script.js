...
// when i click on the volume icon then only the volume range will be visible
let volumeIcon = document.getElementById('soundIcon');
let volumeRangeDivVisible = false;
let timeoutId;
...
Array.from(document.getElementsByClassName('songItemPlay')).forEach((element)=>{
    element.addEventListener('click', (e)=>{ 
        songIndex = parseInt(e.target.id);
        if(doesplay){
            songImage[songIndex].style.animationPlayState = 'paused';
            isPlaying = false;
        }
        else{
            songImage[songIndex].style.animationPlayState = 'running';
            isPlaying = true;
        }
    });
});
